...
onMounted(() => {
  ...
  if (id) {
    ...
    } else { router.push({ name: 'library' }); return }
  }
...
function getSVGPoint(e: MouseEvent) { return getSVGPointFromClient(e.clientX, e.clientY) }
...
function handleCanvasMouseDown(e: MouseEvent) {
  ...
  if (isBg) {
    if (isCreating.value) {
      ...
      selectedNodeId.value = null; showColorPicker.value = false
    }
  }
}
...
function handleNodeMouseDown(e: MouseEvent, node: SkillNode) {
  ...
  selectedNodeId.value = node.id; showColorPicker.value = false
}
...
function handleNodeDblClick(e: MouseEvent, node: SkillNode) { e.stopPropagation(); e.preventDefault(); renameNode(node) }

function renameNode(node: SkillNode) {
  const newTitle = prompt(t('editorPromptRename'), node.title)
  if (newTitle && newTitle.trim()) { pushHistory(); node.title = newTitle.trim() }
}
...
function addResource() { resourcesDraft.value.push({ title: '', url: '', type: 'article' }) }
function removeResource(index: number) { resourcesDraft.value.splice(index, 1) }
...
function saveNodeSettings() {
  ...
  if (node) {
    ...
    const validResources = resourcesDraft.value.filter(r => r.title.trim() && r.url.trim()).map(r => ({ title: r.title.trim(), url: r.url.trim(), type: r.type as Resource['type'] }))
...
function setNodeColor(color: string) {
  if (!selectedNodeId.value) return
  const node = tree.value.nodes.find(n => n.id === selectedNodeId.value)
  if (node) { pushHistory(); node.color = node.color === color ? undefined : color }
}

function handleKeydown(e: KeyboardEvent) {
  if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement) return
  if (e.ctrlKey && e.key === 'z') { e.preventDefault(); undo() }
  else if (e.key === 'Delete' || e.key === 'Backspace') { if (selectedNodeId.value) { e.preventDefault(); deleteSelected() } }
  else if (e.key === 'F2') { if (selectedNodeId.value) { e.preventDefault(); const node = tree.value.nodes.find(n => n.id === selectedNodeId.value); if (node) renameNode(node) } }
  else if (e.key === 'Escape') {
    isCreating.value = false; selectedNodeId.value = null; connectingFrom.value = null
    showNodeSettingsModal.value = false; showColorPicker.value = false; showMetaModal.value = false; showToolsSheet.value = false; showColorPickerSheet.value = false
  }
}
...
function saveTree() {
  ...
  isDirty.value = false; router.push({ name: 'library' })
}
...
<template>
  <div class="flex flex-col h-[calc(100vh-65px)] bg-gray-100 dark:bg-gray-900">
    ...
    <div class="flex-1 overflow-hidden relative touch-none" :class="{ 'cursor-crosshair': isCreating, 'cursor-grab': !isCreating && !draggingNode }">
      <svg ref="svgRef" class="w-full h-full touch-none" :viewBox="`${viewBox.x} ${viewBox.y} ${viewBox.w} ${viewBox.h}`" @mousedown="handleCanvasMouseDown" @mousemove="handleMouseMove" @mouseup="handleMouseUp" @mouseleave="handleMouseUp" @wheel.prevent="handleWheel" @touchstart.prevent="handleCanvasTouchStart" @touchmove.prevent="handleCanvasTouchMove" @touchend.prevent="handleCanvasTouchEnd" @contextmenu.prevent>
        <defs>
        ...
        <path v-if="connectingFrom" :d="ghostLinePath" fill="none" stroke="#3b82f6" stroke-width="2" stroke-dasharray="6,6" stroke-linecap="round" class="pointer-events-none" />
        <g v-for="node in tree.nodes" :key="node.id" :data-node-id="node.id" :transform="`translate(${node.x},${node.y})`" class="node-group" :class="draggingNode === node.id ? 'cursor-grabbing' : 'cursor-grab'" @mousedown="handleNodeMouseDown($event, node)" @dblclick="handleNodeDblClick($event, node)" @touchstart="handleNodeTouchStart($event, node)">
          <rect x="-70" y="-24" width="140" height="48" rx="12" class="transition-all duration-200" :class="{ 'stroke-blue-500 dark:stroke-blue-400 stroke-2 shadow-lg': selectedNodeId === node.id, 'stroke-gray-400 dark:stroke-gray-600 stroke-1.5 shadow-sm': selectedNodeId !== node.id }" :fill="node.color || (isDark ? '#1f2937' : '#ffffff')" />
          <text x="0" y="5" text-anchor="middle" class="text-sm font-semibold pointer-events-none select-none" :fill="node.color ? '#ffffff' : (isDark ? '#f3f4f6' : '#111827')">{{ node.title.length > 16 ? node.title.substring(0, 16) + '...' : node.title }}</text>
          <circle cx="70" cy="0" r="6" class="fill-blue-500 dark:fill-blue-400 stroke-white dark:stroke-gray-800 stroke-2 cursor-crosshair opacity-0 hover:opacity-100 transition-opacity" :class="{ 'opacity-100': selectedNodeId === node.id }" @mousedown="handleHandleMouseDown($event, node)" />
        </g>
      </svg>
      ...
      <div class="hidden sm:flex absolute top-4 left-1/2 -translate-x-1/2 bg-white dark:bg-gray-800 shadow-xl rounded-xl p-2 gap-2 border border-gray-300 dark:border-gray-700 z-20">
        <button @click="undo" :disabled="history.length === 0" class="px-4 py-2 rounded-lg text-sm font-medium transition bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 disabled:opacity-40 disabled:cursor-not-allowed">{{ t('editorUndo') }}</button>
        <button @click="isCreating = !isCreating" class="px-4 py-2 rounded-lg text-sm font-medium transition" :class="isCreating ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600'">{{ t('editorCreate') }}</button>
        <button @click="autoLayout" :disabled="tree.nodes.length === 0" class="px-4 py-2 rounded-lg text-sm font-medium transition bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 disabled:opacity-40 disabled:cursor-not-allowed">{{ t('autoLayout') }}</button>
        <button @click="selectedNodeId && renameNode(tree.nodes.find(n => n.id === selectedNodeId)!)" :disabled="!isRenameActive" class="px-4 py-2 rounded-lg text-sm font-medium transition bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 disabled:opacity-40 disabled:cursor-not-allowed">{{ t('editorRename') }}</button>
        <button @click="openNodeSettings" :disabled="!isSettingsActive" class="px-4 py-2 rounded-lg text-sm font-medium transition bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 disabled:opacity-40 disabled:cursor-not-allowed">{{ t('nodeSettings') }}</button>
        <div class="relative">
          <button @click="showColorPicker = !showColorPicker" :disabled="!isColorActive" class="px-4 py-2 rounded-lg text-sm font-medium transition bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 disabled:opacity-40 disabled:cursor-not-allowed">{{ t('nodeColor') }}</button>
          <div v-if="showColorPicker" class="absolute top-full left-0 mt-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-xl p-2 flex gap-1.5 z-30">
            <button v-for="color in NODE_COLORS" :key="color" @click="setNodeColor(color)" class="w-7 h-7 rounded-full border-2 transition-transform hover:scale-110" :class="selectedNodeId && tree.nodes.find(n => n.id === selectedNodeId)?.color === color ? 'border-gray-900 dark:border-white scale-110' : 'border-transparent'" :style="{ backgroundColor: color }" />
