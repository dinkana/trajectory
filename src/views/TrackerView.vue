...
function triggerPulse(nodeId: string) {
  if (!tree.value) return
  const outgoing = tree.value.edges.filter(e => e.from === nodeId)
  outgoing.forEach(e => {
...
<template>
  <div v-if="tree" class="flex flex-col h-[calc(100vh-65px)] bg-gray-50 dark:bg-gray-900">
    <div class="p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center gap-4 flex-wrap">
      ...
      <div v-if="settingsStore.streak > 0" class="flex items-center gap-1.5 px-3 py-2 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 rounded-lg text-sm font-medium border border-orange-200 dark:border-orange-800/50">
        🔥 {{ settingsStore.streak }} {{ t('streakDays') }}
      </div>

      <button 
        @click="exportToPng" 
        class="px-3 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition"
      >
        {{ t('exportPng') }}
      </button>
      ...
      <button 
        ...
        class="px-3 py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-gray-500"
      >
        {{ t('trackerReset') }}
      </button>
    </div>
      ...
      >
        ...
        <g>
          <template v-for="edge in tree.edges" :key="edge.id">
            <path
              ...
              :class="isCompleted(edge.from) ? 'stroke-emerald-500 dark:stroke-emerald-400' : 'stroke-gray-300 dark:stroke-gray-700'"
            />
            <path
        ...
        >
          <rect
            ...
            stroke-width="2"
            :class="{
              'fill-emerald-500 dark:fill-emerald-600 stroke-emerald-600 dark:stroke-emerald-500': isCompleted(node.id),
              'fill-white dark:fill-gray-800 stroke-blue-500 dark:stroke-blue-400': !isCompleted(node.id) && isUnlocked(node.id),
              'fill-gray-100 dark:fill-gray-800 stroke-gray-300 dark:stroke-gray-700': !isCompleted(node.id) && !isUnlocked(node.id)
            }"
          />
          ...
          <text
            ...
            :class="{
              'fill-white': isCompleted(node.id),
              'fill-gray-900 dark:fill-gray-100': !isCompleted(node.id) && isUnlocked(node.id),
              'fill-gray-400 dark:fill-gray-500': !isCompleted(node.id) && !isUnlocked(node.id)
            }"
          >
          ...
          <foreignObject 
            v-if="hoveredNodeId === node.id && node.description" 
            :x="-100" 
            :y="30" 
            width="200" 
            height="100" 
    ...
    <div v-if="showWinModal" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-8 text-center border border-gray-200 dark:border-gray-700 animate-win-modal">
        ...
        <button 
          ...
          class="w-full px-4 py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition font-medium shadow-lg shadow-emerald-500/20"
        >
          {{ t('continue') }}
        </button>
      </div>
